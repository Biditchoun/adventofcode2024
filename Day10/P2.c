...
//Code of the day
int	recursive_pathing(char **input, int i, int j, char c)
{
	int	rt;
	...
	if (i > 0 && input[i-1][j] == c)
		rt += recursive_pathing(input, i-1, j, c);
	if (j > 0 && input[i][j-1] == c)
		rt += recursive_pathing(input, i, j-1, c);
...
int	main()
{
	...
	while (input[++i]) {
		...
		while (input[i][++j])
			if (input[i][j] == '0')
				rt += recursive_pathing(input, i, j, '0');
	}
	ffree((void **)input);
	printf("%i\n", rt);
}
