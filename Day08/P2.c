...
void	place_antinode_line(char **antinodes_map, int *coord1, int* coord2)
{
	int	a, b, i, j, imax, jmax;
	...
	while (antinodes_map[0][jmax])
		jmax++;
	a = (coord2[0] - coord1[0]) / GCD(abs(coord1[0] - coord2[0]), abs(coord1[1] - coord2[1]));
	b = (coord2[1] - coord1[1]) / GCD(abs(coord1[0] - coord2[0]), abs(coord1[1] - coord2[1]));
	i = coord1[0];
	j = coord1[1];
	while (i > -1 && j > -1 && i < imax && j < jmax) {
		antinodes_map[i][j] = '#';
		i -= a;
		j -= b;
	}
	...
	while (chars[++i]) {
		...
		if (char_coord[0][0] > 1) {
			...
			while (char_coord[++j+1]) {
				k = j;
				while (char_coord[++k])
					place_antinode_line(antinodes_map, char_coord[j], char_coord[k]);
			}
		}
