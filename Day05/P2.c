...
//Code of the day
int	check_bad_update(int **rules, int *update)
{
	int	i, i2, j;
	...
	while (update[++i] > INT_MIN && update[i+1] > INT_MIN) {
		...
		while (rules[++j]) {
			if (rules[j][1] == update[i]) {
				i2 = i;
				while (update[++i2] > INT_MIN)
					if (rules[j][0] == update[i2])
						return (j);
			}
		}
	}
...
int	sort_bad_update(int **rules, int *update)
{
	int	i, j;

	j = check_bad_update(rules, update);
	while (j > INT_MIN) {
	dintplay(update);
	printf("%i|%i\n", rules[j][0], rules[j][1]);
		i = 0;
		while (rules[j][1] != update[i])
			i++;
		update[i++] = rules[j][0];
		while (rules[j][0] != update[i])
			i++;
		update[i] = rules[j][1];
		j = check_bad_update(rules, update);
	}
	dintplay(update);
	return (get_mid_list(update));
}
...
int	main()
{
	...
	j = -1;
	while (updates[++j])
		if (check_bad_update(rules, updates[j]) > INT_MIN)
			rt += sort_bad_update(rules, updates[j]);
	printf("%i\n", rt);
	rules[i] = buff;
	ffree((void **)rules);
}
