...
int	file_size(int *line, int j)
{
	...
	buff = line[j];
	rt = 1;
	while (--j > -1 && line[j] == buff)
		rt++;
	return (rt);
}

int	find_fitting_space(int *line, int j, int j_space)
{
	int	i, i_space;
...
void	compact_line(int *line)
{
	...
	while (j) {
		while (j && line[j] < 0)
			j--;
		j_space = file_size(line, j);
		i = find_fitting_space(line, j, j_space);
		if (i > -1) {
			buff = j_space;
			while (buff--)
				line[i++] = line[j];
